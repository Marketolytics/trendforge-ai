...
@router.post("/research/{trend_id}", response_model=AIEnvelope)
async def generate_research(trend_id: int, force: bool = Query(False)) -> AIEnvelope:
    return await _run(analyzer.generate(trend_id, "research", force=force))
